...
DMainScreen::DMainScreen(QWidget *parent): QGLWidget(QGLFormat(QGL::SampleBuffers), parent)
    {
        ...
        //--------------------------------
        paint_timer_interval = 33;
        tick_timer_interval = 33;

        painttimer = this->startTimer(paint_timer_interval,Qt::PreciseTimer);
        ticktimer = this->startTimer(tick_timer_interval,Qt::PreciseTimer);
            ...
            for (uint j=0;j<HEIGHT;j++)
                {
                    ...
                    bool same = true;
                    do {
                       cells[i][j].variety = DRandom()%8;

                       bool sameh;
                       if (i>0)
                           sameh = cells[i][j].variety == cells[i-1][j].variety;
                       else
                           sameh = false;

                       bool samev;
                       if (j>0)
                           samev = cells[i][j].variety == cells[i][j-1].variety;
                       else
                           samev = false;

                       bool samed;
                       if ((j>0)&&(i>0))
                           samed = cells[i][j].variety == cells[i-1][j-1].variety;
                       else
                           samed = false;

                       same = sameh || samev || samed;
        ...
        if (event->timerId() == ticktimer)
            {
                ...
                if (!LocoList.isEmpty())
                    {
                        for (QList<DLoco>::iterator loco = LocoList.begin();loco!=LocoList.end();loco++)
                            {
                                loco->t = loco->t + loco->v;

                                DPathPoint * lastpoint = &(loco->path->points.last());
                                const DPathPoint * nextpoint = &(loco->path->points.at(loco->nextpointindex));

                                if (loco->t > lastpoint->t)
                                    {
                                        //overflow to next tile
                                        double pre = (loco->t - loco->v);
                                        double togo = nextpoint->t - pre;

                                        loco->y = loco->y + togo * sina[loco->d];
                                        loco->x = loco->x + togo * cosa[loco->d];

                                        double post = loco->t - nextpoint->t;

                                       // TODO loco->path = NEWPATH (PATH FINDING HERE)

                                        switch (loco->d)
                                            {
                                            case dir_r:
                                                {
                                                    DVerticalSideNode *node = static_cast<DVerticalSideNode*>(loco->path->end);
                                                    loco->path = node->path_r_main;
                                                    break;
                                                }

                                            case dir_ur:
                                                {
                                                    DCornerNode *node = static_cast<DCornerNode*>(loco->path->end);
                                                    loco->path = node->path_ur_main;
                                                    break;
                                                }
                                            ...
                                            case dir_u:
                                                {
                                                    ...
                                                    DHorizontalSideNode *node = static_cast<DHorizontalSideNode*>(loco->path->end);
                                                    loco->path = node->path_u_main;
                                                    break;
                                                }
                                            ...
                                            case dir_dl:
                                                {
                                                    DCornerNode *node = static_cast<DCornerNode*>(loco->path->end);
                                                    loco->path = node->path_dl_main;
                                                    break;
                                                }
                                            case dir_d:
                                                {
                                                    DHorizontalSideNode *node = static_cast<DHorizontalSideNode*>(loco->path->end);
                                                    loco->path = node->path_d_main;
                                                    break;
                                                }
                                        ...
                                        if (loco->path!=nullptr)
                                            {

                                        loco->nextpointindex = 1;
                                        loco->d = loco->path->points.first().newdir;
                                        //go along new direction
                                        loco->y = loco->y + post * sina[loco->d];
                                        loco->x = loco->x + post * cosa[loco->d];
                                        loco->t = post;
                                            }

                                    }
                                else {
                                        if (loco->t > nextpoint->t)
                                            {
                                                //overflow to next path section
                                                double pre = (loco->t - loco->v);
                                                double togo = nextpoint->t - pre;
                                                ...
                                                // determine how much to go
                                                double post = loco->t - nextpoint->t;
                                                //change direction
                                                loco->d = nextpoint->newdir;
                                                //go along new direction
                                                loco->y = loco->y + post * sina[loco->d];
                                                loco->x = loco->x + post * cosa[loco->d];
                                                ...
                                                // move along
                                                loco->y = loco->y + loco->v * sina[loco->d];
                                                loco->x = loco->x + loco->v * cosa[loco->d];
                                            }
                ...
                while (1)
                    {
                        ...
                        if ((CurrentTool==TOOL_TEMP_SPAWNLOCO) && (LastHoverGroup==HOVER_GROUP_TERRAIN) && ( (obstructions[cursor_x][cursor_y].u==iter)||(obstructions[cursor_x][cursor_y].l==iter)||(obstructions[cursor_x][cursor_y].r==iter)||(obstructions[cursor_x][cursor_y].d==iter)))
                            {
                                QColor tint;tint.setRgbF(0.0,1.0,0.0);
                                shader->setUniformValue(shader->uniformLocation("tint"),tint);
                                shader->setUniformValue(shader->uniformLocation("allwhite"),1);
                                shader->setUniformValue(shader->uniformLocation("noshade"),1);
                            }
                ...
                for (QList<DLoco>::iterator loco= LocoList.begin();loco!=LocoList.end();loco++)
                    {
                        ...
                        degr = -(loco->d * 15) + 90;
                        degr = (degr+360)%360;
                        RENDER_DRAW_3D_CAR(loco->x, loco->y, degr);
                    }
            }
...
void DMainScreen::RENDER_DRAW_3D_CAR(GLfloat i, GLfloat j, GLfloat degree)
    {
        glColor3f(1.0,1.0,1.0);
...
/*
        ...
        */
        ...
        GLfloat centroid_x = sx + w/2*i -w/2*j;
        GLfloat centroid_y = sy + w/4*i + w/4*j;
...
void DMainScreen::keyPressEvent(QKeyEvent *event)
    {
        ...
        switch (event->key())
            ...
            case Qt::Key_L:
                CurrentTool = TOOL_TEMP_SPAWNLOCO;
                break;
